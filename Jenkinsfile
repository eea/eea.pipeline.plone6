pipeline {
  ...
  stages {
    ...
    stage('BISE-BACKEND') {
    stages {
      stage('Build & Tests - BISE-BACKEND - new PLONE-BACKEND release') {
        steps {
          build job: '../bise-backend/master', parameters: [[$class: 'StringParameterValue', name: 'TARGET_BRANCH', value: 'master']]
        }
      }
      stage('Release - BISE-BACKEND') {
        steps {
          node(label: 'docker') {
            withCredentials([string(credentialsId: 'eea-jenkins-token', variable: 'GITHUB_TOKEN'), string(credentialsId: 'bise-backend-trigger', variable: 'TRIGGER_MAIN_URL'),usernamePassword(credentialsId: 'jekinsdockerhub', usernameVariable: 'DOCKERHUB_USER', passwordVariable: 'DOCKERHUB_PASS')]) {
              sh '''docker pull eeacms/gitflow; docker run -i --rm --name="$BUILD_TAG-nightly-bise-back" -e GIT_BRANCH="master" -e GIT_NAME="bise-backend" -e EXTRACT_VERSION_SH="calculate_next_release.sh" -e GIT_TOKEN="$GITHUB_TOKEN" -e TRIGGER_MAIN_URL="$TRIGGER_MAIN_URL" -e DOCKERHUB_USER="$DOCKERHUB_USER" -e DOCKERHUB_PASS="$DOCKERHUB_PASS" -e DOCKERHUB_REPO="eeacms/bise-backend" -e DEPENDENT_DOCKERFILE_URL=""  -e TRIGGER_RELEASE="" -e GITFLOW_BEHAVIOR="TAG_ONLY" eeacms/gitflow'''
            }
          }
        }
      }
    ...
    stage('MARINE-BACKEND') {
    stages {
      stage('Build & Tests - MARINE-BACKEND - new PLONE-BACKEND release') {
        steps {
          build job: '../marine-backend/master', parameters: [[$class: 'StringParameterValue', name: 'TARGET_BRANCH', value: 'master']]
        }
      }
      stage('Release - MARINE-BACKEND') {
        steps {
          node(label: 'docker') {
            withCredentials([string(credentialsId: 'eea-jenkins-token', variable: 'GITHUB_TOKEN'), string(credentialsId: 'marine-backend-trigger', variable: 'TRIGGER_MAIN_URL'),usernamePassword(credentialsId: 'jekinsdockerhub', usernameVariable: 'DOCKERHUB_USER', passwordVariable: 'DOCKERHUB_PASS')]) {
              sh '''docker pull eeacms/gitflow; docker run -i --rm --name="$BUILD_TAG-nightly-marine-back" -e GIT_BRANCH="master" -e GIT_NAME="marine-backend" -e EXTRACT_VERSION_SH="calculate_next_release.sh" -e GIT_TOKEN="$GITHUB_TOKEN" -e TRIGGER_MAIN_URL="$TRIGGER_MAIN_URL" -e DOCKERHUB_USER="$DOCKERHUB_USER" -e DOCKERHUB_PASS="$DOCKERHUB_PASS" -e DOCKERHUB_REPO="eeacms/marine-backend" -e DEPENDENT_DOCKERFILE_URL=""  -e TRIGGER_RELEASE="" -e GITFLOW_BEHAVIOR="TAG_ONLY" eeacms/gitflow'''
            }
          }
        }
      }
